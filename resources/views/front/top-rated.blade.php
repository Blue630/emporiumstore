...
<li class="breadcrumb-item active" aria-current="page">Top Rated</li>
</ol>
</nav>
</div>
...
$category = DB::table('category')->get();
$subcategory = DB::table('sub_category')->get();
$topratedproduct = DB::table('review')->where('rating','>=',3)->groupBy('product_id')->get();
$newproduct_id='';
foreach($topratedproduct as $topratedproduct_result)
{
$product_id = $topratedproduct_result->product_id;
$topproduct = DB::table('products')->where(array('id'=>$product_id,'status'=>1))->orderBy('price','desc')->get();
foreach($topproduct as $topproduct_result)
{
$product_id = $topproduct_result->id;
?>
<div class="col-lg-3 col-md-4 col-6">
<div class="products_unit">
<div class="image">
<a href="{{url('product-detail/'.$topproduct_result->slug)}}"><img src="{{ asset('public/products/'.$topproduct_result->image) }}" class="img-fluid" alt=""></a>
</div>
...
<button title="Add to Wish List" data-toggle="tooltip" type="button" class="btn-wishlist" href="javascript:void(0);" onClick="addwishlist('<?php echo $topproduct_result->id; ?>');">
    <?php
    $wishlist = DB::table('wishlist')->where(array('user_id'=>$user_id,'status'=>1,'product_id'=>$topproduct_result->id))->get();
...
<div class="p_name">
<a href="{{url('product-detail/'.$topproduct_result->slug)}}">{{$topproduct_result->name}}</a>
</div>
<div class="p_rating d-flex align-items-center gap-2">
@php
$rating_data = Review::getRatingAverage($topproduct_result->id);
@endphp
...
<div class="p_price">
<i class="fas fa-pound-sign"></i> {{$topproduct_result->price}}
</div>
<?php
...
{
$product_id = $topratedproduct_result->product_id;
$topproduct = DB::table('products')->where(array('id'=>$product_id,'status'=>1))->orderBy('id',$orderby)->get();
foreach($topproduct as $topproduct_result)
{
$product_id = $topproduct_result->id;
?>
...
{
$product_id = $topratedproduct_result->product_id;
$topproduct = DB::table('products')->where(array('id'=>$product_id,'status'=>1))->whereIn('id', $newproductid)->orderBy('id','desc')->get();
foreach($topproduct as $topproduct_result)
{
$product_id = $topproduct_result->id;
?>
...
{
$product_id = $topratedproduct_result->product_id;
$topproduct = DB::table('products')->where(array('id'=>$product_id,'status'=>1))->orderBy('id','desc')->get();
foreach($topproduct as $topproduct_result)
{
$product_id = $topproduct_result->id;
?>
