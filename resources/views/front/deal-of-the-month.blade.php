...
<div class="row">
<?php
$monthlydealsproductcount = count($monthlydealsproduct);
if($monthlydealsproductcount>0)
{
foreach ($monthlydealsproduct as $monthlydealsproduct_result) 
{
$product_id = $monthlydealsproduct_result->id;
?>
<div class="col-lg-3 col-md-4 col-6">
<div class="products_unit">
<div class="image">
<a href="{{url('product-detail/'.$monthlydealsproduct_result->slug)}}"><img src="{{ asset('public/products/'.$monthlydealsproduct_result->image) }}" class="img-fluid" alt=""></a>
</div>
...
<button title="Add to Wish List" data-toggle="tooltip" type="button" class="btn-wishlist" href="javascript:void(0);" onClick="addwishlist('<?php echo $monthlydealsproduct_result->id; ?>');">
    <?php
    $wishlist = DB::table('wishlist')->where(array('user_id'=>$user_id,'status'=>1,'product_id'=>$monthlydealsproduct_result->id))->get();
...
<div class="p_name">
<a href="{{url('product-detail/'.$monthlydealsproduct_result->slug)}}">{{$monthlydealsproduct_result->name}}</a>
</div>
<div class="p_rating d-flex align-items-center gap-2">
@php
$rating_data = Review::getRatingAverage($monthlydealsproduct_result->id);
@endphp
...
<div class="p_price">
<i class="fas fa-pound-sign"></i> {{$monthlydealsproduct_result->price}}
</div>
<?php
