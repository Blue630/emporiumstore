...
<div class="row">
<?php
$weeklydealsproductcount = count($weeklydealsproduct);
if($weeklydealsproductcount>0)
{
...
<div class="image">
<a href="{{url('product-detail/'.$weeklydealsproduct_result->slug)}}"><img src="{{ asset('public/products/'.$weeklydealsproduct_result->image) }}" class="img-fluid" alt=""></a>
</div>
...
<button title="Add to Wish List" data-toggle="tooltip" type="button" class="btn-wishlist" href="javascript:void(0);" onClick="addwishlist('<?php echo $weeklydealsproduct_result->id; ?>');">
    <?php
    $wishlist = DB::table('wishlist')->where(array('user_id'=>$user_id,'status'=>1,'product_id'=>$weeklydealsproduct_result->id))->get();
...
<div class="p_name">
<a href="{{url('product-detail/'.$weeklydealsproduct_result->slug)}}">{{$weeklydealsproduct_result->name}}</a>
</div>
<div class="p_rating d-flex align-items-center gap-2">
@php
$rating_data = Review::getRatingAverage($weeklydealsproduct_result->id);
@endphp
...
<div class="p_price">
<i class="fas fa-pound-sign"></i> {{$weeklydealsproduct_result->price}}
</div>
<div class="p_offer">
{{$weeklydealsproduct_result->cashback}}% Cashback
</div>
</div>
</div>
</div>
