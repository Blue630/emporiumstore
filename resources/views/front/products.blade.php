...
{
?>
<a href="{{url('category/'.$category_value->slug)}}" class="ft-18 lh-27"><i data-feather="chevron-left"></i> {{$category_value->catname}}</a>
<?php
}
}
?>
...
{
$product_id = $products_value->id;
?>
<div class="col-lg-3 col-md-4 col-6">
<div class="products_unit">
<div class="image">
<a href="{{url('product-detail/'.$products_value->slug)}}"><img src="{{ asset('public/products/'.$products_value->image) }}" class="img-fluid" alt=""></a>
</div>
...
<button title="Add to Wish List" data-toggle="tooltip" type="button" class="btn-wishlist" href="javascript:void(0);" onClick="addwishlist('<?php echo $products_value->id; ?>');">
    <?php
    $wishlist = DB::table('wishlist')->where(array('user_id'=>$user_id,'status'=>1,'product_id'=>$products_value->id))->get();
...
<div class="p_name">
<a href="{{url('product-detail/'.$products_value->slug)}}">{{$products_value->name}}</a>
</div>
<div class="p_rating d-flex align-items-center gap-2">
@php
$rating_data = Review::getRatingAverage($products_value->id);
@endphp
...
<div class="p_price">
<i class="fas fa-pound-sign"></i> {{$products_value->price}}
</div>
<?php
