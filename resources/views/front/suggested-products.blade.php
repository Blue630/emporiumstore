...
<div class="image">
<a href="{{url('product-detail/'.$product->slug)}}"><img src="{{ asset('public/products/'.$product->image) }}" class="img-fluid" alt=""></a>
</div>
...
<button title="Add to Wish List" data-toggle="tooltip" type="button" class="btn-wishlist" href="javascript:void(0);" onClick="addwishlist('<?php echo $product->id; ?>');">
    <?php
    $wishlist = DB::table('wishlist')->where(array('user_id'=>$user_id,'status'=>1,'product_id'=>$product->id))->get();
...
@php
$rating_data = Review::getRatingAverage($product->id);
@endphp
...
<div class="p_price">
<i class="fas fa-pound-sign"></i> {{$product->price}}
</div>
<?php
