...
<div class="row">
<?php
$bestsellerproductcount = count($bestsellerproduct);
if($bestsellerproductcount>0)
{
foreach ($bestsellerproduct as $bestsellerproduct_result) 
{
$product_id = $bestsellerproduct_result->id;
?>
<div class="col-lg-3 col-md-4 col-6">
<div class="products_unit">
<div class="image">
<a href="{{url('product-detail/'.$bestsellerproduct_result->slug)}}"><img src="{{ asset('public/products/'.$bestsellerproduct_result->image) }}" class="img-fluid" alt=""></a>
</div>
...
<button title="Add to Wish List" data-toggle="tooltip" type="button" class="btn-wishlist" href="javascript:void(0);" onClick="addwishlist('<?php echo $bestsellerproduct_result->id; ?>');">
    <?php
    $wishlist = DB::table('wishlist')->where(array('user_id'=>$user_id,'status'=>1,'product_id'=>$bestsellerproduct_result->id))->get();
...
<div class="p_name">
<a href="{{url('product-detail/'.$bestsellerproduct_result->slug)}}">{{$bestsellerproduct_result->name}}</a>
</div>
<div class="p_rating d-flex align-items-center gap-2">
@php
$rating_data = Review::getRatingAverage($bestsellerproduct_result->id);
@endphp
...
<div class="p_price">
<i class="fas fa-pound-sign"></i> {{$bestsellerproduct_result->price}}
</div>
<?php
