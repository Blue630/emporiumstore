...
<li class="breadcrumb-item active" aria-current="page">Subscribe & Save</li>
</ol>
</nav>
</div>
...
$category = DB::table('category')->get();
$subcategory = DB::table('sub_category')->get();
$subscribe_seller = DB::table('seller_subscriptions')->where('end_date','>=',date('Y-m-d'))->groupBy('seller_id')->get();
foreach($subscribe_seller as $subscribe_seller_result)
{
$seller_id = $subscribe_seller_result->seller_id;
$products = DB::table('products')->where('user_id',$seller_id)->orderBy('price',$orderby)->get();
...
{
$seller_id = $subscribe_seller_result->seller_id;
$products = DB::table('products')->where('user_id',$seller_id)->orderBy('id',$orderby)->get();
foreach($products as $product_result)
{
$product_id = $product_result->id;
?>
...
{
$seller_id = $subscribe_seller_result->seller_id;
$products = DB::table('products')->where('user_id',$seller_id)->whereIn('id', $newproductid)->orderBy('id','desc')->get();
foreach($products as $product_result)
{
$product_id = $product_result->id;
?>
