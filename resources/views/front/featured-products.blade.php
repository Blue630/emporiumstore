...
$category = DB::table('category')->get();
$subcategory = DB::table('sub_category')->get();
$featuredproduct = DB::table('products')->where(array('status'=>1,'is_featured'=>1))->orWhere('is_admin_featured',1)->orderBy('price',$orderby)->limit(10)->get();
$currentDate = date('Y-m-d');
foreach($featuredproduct as $featuredproduct_result)
{
$product_id = $featuredproduct_result->id;
$seller_id = $featuredproduct_result->user_id;
$newseller_id = explode(',', $seller_id);
$checkfeaturedproductdate = DB::table('seller_featured_subscription')->where('seller_id',$newseller_id)->first();
if(!empty($checkfeaturedproductdate))
{
$end_date = $checkfeaturedproductdate_result->end_date;
...
<div class="image">
<a href="{{url('product-detail/'.$featuredproduct_result->slug)}}"><img src="{{ asset('public/products/'.$featuredproduct_result->image) }}" class="img-fluid" alt=""></a>
</div>
...
<button title="Add to Wish List" data-toggle="tooltip" type="button" class="btn-wishlist" href="javascript:void(0);" onClick="addwishlist('<?php echo $featuredproduct_result->id; ?>');">
    <?php
    $wishlist = DB::table('wishlist')->where(array('user_id'=>$user_id,'status'=>1,'product_id'=>$featuredproduct_result->id))->get();
...
<div class="p_name">
<a href="{{url('product-detail/'.$featuredproduct_result->slug)}}">{{$featuredproduct_result->name}}</a>
</div>
<div class="p_rating d-flex align-items-center gap-2">
@php
$rating_data = Review::getRatingAverage($featuredproduct_result->id);
@endphp
...
<div class="p_price">
<i class="fas fa-pound-sign"></i> {{$featuredproduct_result->price}}
</div>
<?php
...
$category = DB::table('category')->get();
$subcategory = DB::table('sub_category')->get();
$featuredproduct = DB::table('products')->where(array('status'=>1,'is_featured'=>1))->orWhere('is_admin_featured',1)->orderBy('id',$orderby)->limit(10)->get();
$currentDate = date('Y-m-d');
foreach($featuredproduct as $featuredproduct_result)
{
$seller_id = $featuredproduct_result->user_id;
...
$category = DB::table('category')->get();
$subcategory = DB::table('sub_category')->get();
$featuredproduct = DB::table('products')->where(array('status'=>1,'is_featured'=>1))->orWhere('is_admin_featured',1)->whereIn('id', $newproductid)->orderBy('id','desc')->get();
$currentDate = date('Y-m-d');
foreach($featuredproduct as $featuredproduct_result)
{
$product_id = $featuredproduct_result->id;
...
{
$featuredproduct = DB::table('products')->where(array('status'=>1,'is_featured'=>1))->orWhere('is_admin_featured',1)->orderBy('id','desc')->get();
$currentDate = date('Y-m-d');
foreach($featuredproduct as $featuredproduct_result)
{
$product_id = $featuredproduct_result->id;
