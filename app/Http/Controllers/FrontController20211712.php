...
class FrontController extends Controller
{
    ...
    public function  product_list(Request $req)
    {
        ...
        return view('/front/product-list',['product'=>$product,'cate'=>$cate,'type'=>$type,'search_no'=>$search_no]);
        }
        
        $product=DB::table('products')->where('status',1)->orderBy('id','desc')->paginate(9);
        $cate=DB::table('category')->orderBy('id','desc')->get();
        return view('/front/product-list',['product'=>$product,'cate'=>$cate]);
    }
   ...
   public function registeration(Request $req)
    ...
    function filterproduct($slug)
    {
        ...
        $cate=DB::table('category')->orderBy('id','desc')->get();
        return view('/front/product-list',['product'=>$filter,'cate'=>$cate]);
    }
    ...
    function filter_seach(Request $req)
    {
        ...
        $cate=DB::table('category')->orderBy('id','desc')->get();
        return view('/front/product-list',['product'=>$filter,'cate'=>$cate,'type'=>$type,'search_no'=>$search_no]);
    }
